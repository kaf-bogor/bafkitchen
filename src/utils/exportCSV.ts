...
export const exportOrdersToCSV = (
  ...
  if (link.download !== undefined) {
    ...
    link.setAttribute(
      'download',
      filename || `Orders_Export_${format(new Date(), 'yyyy-MM-dd_HHmm')}.csv`
    )
...
export const exportDetailedOrdersToCSV = (
    ...
    if (order.productOrders && order.productOrders.length > 0) {
      order.productOrders.forEach((productOrder) => {
        detailedData.push({
          ...
          productTotal: currency.toIDRFormat(
            (productOrder.quantity || 0) * (productOrder.product?.price || 0)
          ),
  ...
  if (link.download !== undefined) {
    ...
    link.setAttribute(
      'download',
      filename ||
        `Orders_Detailed_Export_${format(new Date(), 'yyyy-MM-dd_HHmm')}.csv`
    )
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
...
export const exportTableToCSV = (
  ...
  if (link.download !== undefined) {
    ...
    link.setAttribute(
      'download',
      filename || `Export_${format(new Date(), 'yyyy-MM-dd_HHmm')}.csv`
    )
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
